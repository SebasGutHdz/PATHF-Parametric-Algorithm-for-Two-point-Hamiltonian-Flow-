...
def run_hamiltonian_flow(parametric_model: nnx.Module, 
                        ...
                        G_mat: G_matrix,
                        potential: Potential, 
                        phi_fn: Optional[Callable[[float, Array], Array]] = None,
                        initial_momentum: Optional[PyTree] = None,
                        h: float = 0.01, 
                        solver: str = "cg",
    ...
    if initial_momentum is None and phi_fn is None:
        _, initial_theta = nnx.split(current_parametric_model)
        print('No initial momentum or phi function provided; initializing momentum to zero.')
        current_momentum = jax.tree.map(jnp.zeros_like, initial_theta)
    elif initial_momentum is not None and phi_fn is None:
    ...
    for iteration in p_bar:
        ...
        momentum_norms.append(momentum_norm.astype(float))
        param_norms.append(param_norm.astype(float))
        ...
        #     hamiltonian_drift = (current_hamiltonian - initial_hamiltonian).astype(float)

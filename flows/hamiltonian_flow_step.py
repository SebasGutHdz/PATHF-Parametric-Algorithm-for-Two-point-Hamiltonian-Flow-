...
def hamiltonian_flow_step(parametric_model: ParametricModel, p_n: PyTree, z_samples: Array, G_mat: G_matrix,
                         ...
                         solver_maxiter: int = 50, regularization: float = 1e-6, 
                         gamma: float = 1e-2, n_iters: int = 3,
                         return_derivatives: bool = False) -> Union[Tuple[nnx.Module, PyTree, dict], 
                                                                      Tuple[nnx.Module, PyTree, dict, PyTree, PyTree]]:
    """
    ...
    Args:
        ...
        return_derivatives: If True, also return theta_dot and p_dot
    
    Returns:
        updated_parametric_model: ParametricModel with updated parameters
        updated_momentum: Updated momentum PyTree
        step_info: Dictionary with step diagnostics
        theta_dot: (optional) θ̇ = G^{-1} p if return_derivatives=True
        p_dot: (optional) ṗ if return_derivatives=True
    """
    ...
    alpha, xi = fixed_point_solver(z_samples=z_samples, theta_n=theta_n, p_n=p_n, 
                                   G_mat=G_mat, h=step_size, gamma=gamma, n_iters=n_iters,
                                   solver=solver, maxiter=solver_maxiter, tol=solver_tol, 
    ...
    temp_model = nnx.merge(graph_def, alpha)
    grad_F = jax.grad(lambda theta: potential.evaluate_energy(
        nnx.merge(graph_def, theta), z_samples, theta)[0])(alpha)
    
    # Update momentum: p_{n+1} = p_n + h * (0.5 * grad_G - grad_F)
    ...
    if return_derivatives:
        return updated_parametric_model, p_np1, step_info, xi, p_dot
    else:
        return updated_parametric_model, p_np1, step_info
